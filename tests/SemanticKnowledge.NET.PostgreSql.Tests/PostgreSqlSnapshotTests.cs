...
public sealed class PostgreSqlSnapshotTests
{
    ...
    public async Task Atomic_snapshot_publishes_one_revision_and_failed_replacement_stays_invisible()
    {
        ...
        try
        {
            ...
            phase = "hybrid search revision 2";
        ...
        {
            Console.WriteLine($"::error title=PostgreSQL snapshot failure during {phase}::{Escape(ex.ToString())}");
            throw;
        }
    }

    private static string Escape(string text) => text.Replace("%", "%25", StringComparison.Ordinal).Replace("\r", "%0D", StringComparison.Ordinal).Replace("\n", "%0A", StringComparison.Ordinal);
