...
{
    [Fact]
    public async Task Sqlite_archive_imports_into_sql_server_with_stable_document_identity()
    {
        var cancellationToken = TestContext.Current.CancellationToken;
        var connectionString = Environment.GetEnvironmentVariable("SEMANTIC_KNOWLEDGE_SQLSERVER");
        Assert.False(string.IsNullOrWhiteSpace(connectionString));
        var sqlitePath = Path.Combine(Path.GetTempPath(), $"sk-mssql-migration-{Guid.NewGuid():N}.db");
        try
        {
            ...
            {
                ExternalId = "sqlserver/restore",
                KnowledgeBaseId = kb.Id,
                CollectionId = collection.Id,
                SchemaId = schema.Id,
                Title = "Alpha SQL backup",
                Description = "Portable recovery procedure.",
                Tags = ["alpha", "backup"],
                Values = new Dictionary<string, KnowledgeValue>
                {
                    [KnowledgeSystemFields.Body] = KnowledgeValue.From("alpha sql server backup recovery")
                }
            }, cancellationToken);
            ...
            services.AddSemanticKnowledge(options => options.Embeddings.OutputDimensions = 4)
                .UseSqlServer(options =>
                {
                    ...
                    options.Schema = $"SKArchive{Guid.NewGuid():N}";
                });
            services.AddSingleton<IKnowledgeEmbeddingProvider>(new PortableEmbeddingProvider("sqlserver-target-space"));
            ...
            Assert.Equal("sqlserver/restore", document.ExternalId);
