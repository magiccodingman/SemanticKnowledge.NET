...
internal sealed class PostgreSqlArchiveStorage(NpgsqlDataSource dataSource, SemanticKnowledgePostgreSqlOptions options) : IKnowledgeArchiveStorage
{
    public async Task<KnowledgeBaseRecord?> GetKnowledgeBaseAsync(Guid knowledgeBaseId, CancellationToken cancellationToken = default)
    {
        ...
        await using var command = new NpgsqlCommand($"SELECT external_id,title,description FROM {Q("sk_knowledge_bases")} WHERE id=@id", connection); command.Parameters.AddWithValue("id", knowledgeBaseId);
    ...
    public async Task UpsertKnowledgeBaseAsync(KnowledgeBaseRecord knowledgeBase, CancellationToken cancellationToken = default)
    {
        ...
        await using var command = new NpgsqlCommand($"INSERT INTO {Q("sk_knowledge_bases")}(id,external_id,title,description) VALUES(@id,@external,@title,@description) ON CONFLICT(id) DO UPDATE SET external_id=EXCLUDED.external_id,title=EXCLUDED.title,description=EXCLUDED.description", connection);
