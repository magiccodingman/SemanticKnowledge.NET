...
internal sealed class SqlServerArchiveStorage(SemanticKnowledgeSqlServerOptions options) : IKnowledgeArchiveStorage
{
    public async Task<KnowledgeBaseRecord?> GetKnowledgeBaseAsync(Guid knowledgeBaseId, CancellationToken cancellationToken = default)
    {
        ...
        await using (var command = new SqlCommand($"SELECT external_id,title,description FROM {Q("sk_knowledge_bases")} WHERE id=@id", connection))
        {
            command.Parameters.AddWithValue("@id", knowledgeBaseId); await using var reader = await command.ExecuteReaderAsync(cancellationToken).ConfigureAwait(false); if (!await reader.ReadAsync(cancellationToken).ConfigureAwait(false)) return null;
            externalId = reader.IsDBNull(0) ? null : reader.GetString(0); title = reader.GetString(1); description = reader.GetString(2);
        }
        var tags = new List<string>();
        await using (var command = new SqlCommand($"SELECT tag FROM {Q("sk_knowledge_base_tags")} WHERE knowledge_base_id=@id ORDER BY tag", connection))
    ...
    {
        ValidateKnowledgeBase(knowledgeBase); await using var connection = await OpenAsync(cancellationToken).ConfigureAwait(false); await using var transaction = (SqlTransaction)await connection.BeginTransactionAsync(cancellationToken).ConfigureAwait(false);
        await using (var command = new SqlCommand($"UPDATE {Q("sk_knowledge_bases")} SET external_id=@external,title=@title,description=@description WHERE id=@id; IF @@ROWCOUNT=0 INSERT INTO {Q("sk_knowledge_bases")}(id,external_id,title,description) VALUES(@id,@external,@title,@description);", connection, transaction))
        ...
        await using (var delete = new SqlCommand($"DELETE FROM {Q("sk_knowledge_base_tags")} WHERE knowledge_base_id=@id", connection, transaction)) { delete.Parameters.AddWithValue("@id", knowledgeBase.Id); await delete.ExecuteNonQueryAsync(cancellationToken).ConfigureAwait(false); }
        foreach (var tag in NormalizeTags(knowledgeBase.Tags)) { await using var command = new SqlCommand($"INSERT INTO {Q("sk_knowledge_base_tags")}(knowledge_base_id,tag) VALUES(@id,@tag)", connection, transaction); command.Parameters.AddWithValue("@id", knowledgeBase.Id); command.Parameters.AddWithValue("@tag", tag); await command.ExecuteNonQueryAsync(cancellationToken).ConfigureAwait(false); }
    ...
    {
        var connection = new SqlConnection(options.ConnectionString); await connection.OpenAsync(cancellationToken).ConfigureAwait(false);
        var table = Q("sk_knowledge_base_tags");
        await using var command = new SqlCommand($"IF OBJECT_ID(N'{table}',N'U') IS NULL CREATE TABLE {table}(knowledge_base_id uniqueidentifier NOT NULL,tag nvarchar(450) NOT NULL,PRIMARY KEY(knowledge_base_id,tag)); DELETE t FROM {table} t WHERE NOT EXISTS (SELECT 1 FROM {Q("sk_knowledge_bases")} kb WHERE kb.id=t.knowledge_base_id);", connection); await command.ExecuteNonQueryAsync(cancellationToken).ConfigureAwait(false);
        return connection;
    }
